...
for i in Loc_List:
    ...
    for j in list(df.RIG_NAME.unique()):
        Rig_List_Pass.append(((df[(df['FAIL/PASS']=='Pass')&(df['LOCATION_']==i)&(df['RIG_NAME']==j)]['VALUE'].sum())))
        
    fig=px.bar(x=Rig_List, y=Rig_List_Pass,labels={'x':f'KPC (DRLG/WO) Rigs{i} Pass Points ', 'y':f'Total {i} Pass Points'})

    for k in list(df.RIG_NAME.unique()):
           fig.add_annotation(x=k,y=df[(df['FAIL/PASS']=='Pass')&(df['RIG_NAME']==k)&(df['LOCATION_']==i)]['VALUE'].sum()+2, text=f"{df[(df['FAIL/PASS']=='Pass')&(df['RIG_NAME']==k)&(df['LOCATION_']==i)]['VALUE'].sum()}", showarrow=False)

    fig.update_layout(title_text=f"{df[(df['FAIL/PASS']=='Pass')&(df['LOCATION_']==i)]['VALUE'].sum()} Pass Points with Rig`s Contribution", showlegend=False)
    fig.show()
